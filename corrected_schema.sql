-- =============================================================================
-- DiploChain — Corrected PostgreSQL Schema v5.0
-- Esprit Blockchain Diploma Verification System
--
-- ARCHITECTURE:
--   • Core Esprit tables: preserved EXACTLY as per script_db.sql
--   • Extension tables:   DiploChain-specific augmentation via FK pattern
--   • Support tables:     lookup / reference tables required by core FKs
--   • Blockchain tables:  hash anchoring, QR codes, audit trail
--
-- RULES:
--   ① Core tables are NEVER modified — see Section "PROTECTED CORE TABLES"
--   ② Extra columns go into *_ext or *_blockchain_ext tables
--   ③ Enum types are ONLY used in DiploChain extension tables
--   ④ All extension tables reference core tables via FK
-- =============================================================================
...
-- DiploChain user status — separate from core "User".status (varchar)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- STEP 6 ── ROLE_ID SEQUENCE (required by ROLE table default)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Run migration_script.sql first if upgrading from backup_before_fix.sql
-- =============================================================================
